...
then
    ...
    then
        ...
        echo "curl -X POST -d "$DATA" -H 'Authorization: Bearer ${GITHUB_TOKEN}' -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/ncmuthu/${GITHUB_REPOSITORY}/releases"
        curl -X POST -d "$DATA" -H 'Authorization: Bearer ${GITHUB_TOKEN}' -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/ncmuthu/${GITHUB_REPOSITORY}/releases
    fi
# otherwise
else
    # exit gracefully
